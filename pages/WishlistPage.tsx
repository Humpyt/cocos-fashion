...
const WishlistPage: React.FC<Props> = ({ wishlist, onProductClick, onToggleWishlist, onNavigate }) => {
  return (
            ...
            {wishlist.map(product => (
              <div key={product.id} className="bg-white border border-gray-200 group relative">
                 <div className="relative aspect-[4/5] overflow-hidden cursor-pointer" onClick={() => onProductClick(product)}>
                    ...
                    <button 
                      className="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:bg-cocos-orange hover:text-white transition-all"
                      onClick={(e) => {
                        e.stopPropagation();
