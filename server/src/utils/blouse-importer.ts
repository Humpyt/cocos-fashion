...
import {
  buildBlouseImportGroups,
  parseBlouseFilename,
  type BlouseImportGroup,
  type ParseConfidence,
  type ParsedBlouseFile,
} from "./blouse-import.js";

export type BlouseImportOptions = {
  sourceDir: string;
  dryRun?: boolean;
  verbose?: boolean;
};
...
type CategoryRefs = {
  blousesCategoryId: string;
  womenCategoryId: string;
};

export type BlouseImportSummary = {
...
export type BlouseImportReport = {
  generatedAt: string;
  sourceDir: string;
  dryRun: boolean;
  summary: BlouseImportSummary;
  skippedFiles: Array<{ filename: string; reasons: string[] }>;
  lowConfidence: Array<{
    filename: string;
    confidence: ParseConfidence;
    warnings: string[];
    parsed: Partial<ParsedBlouseFile>;
  }>;
  groups: Array<{
    slug: string;
    brand: string;
...
const ensureCategories = async (): Promise<CategoryRefs> => {
  ...
  return {
    blousesCategoryId: blouses.id,
    womenCategoryId: women.id,
  };
};

const buildProductName = (brand: string, styleDescriptor: string): string =>
  styleDescriptor === "Blouse" ? `${brand} Blouse` : `${brand} ${styleDescriptor} Blouse`;

const buildDescription = (group: BlouseImportGroup): string => {
...
const uniqueSizes = (group: BlouseImportGroup): string[] =>
...
const applyGroup = async (
  ...
  const managedCategoryIds = [categories.blousesCategoryId, categories.womenCategoryId];
    ...
    const product = existing
      ? await tx.product.update({
          ...
          data: {
            ...
            description,
            details: {
              importSource: "blouses-filename-import",
              color: group.color,
              sizes,
            },
            currency: "UGX",
...
export const runBlouseImport = async (options: BlouseImportOptions): Promise<BlouseImportReport> => {
  const sourceDir = resolve(options.sourceDir);
  const dryRun = options.dryRun ?? false;
  const files = await collectFiles(sourceDir);

  const parsedFiles = files.map((filename) => parseBlouseFilename(filename));
  ...
  const groups = buildBlouseImportGroups(parsedFiles);
  ...
  const summary: BlouseImportSummary = {
...
export const writeBlouseImportReport = async (reportPath: string, report: BlouseImportReport) => {
