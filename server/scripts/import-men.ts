...
import { runMenImport, writeMenImportReport } from "../src/utils/men-importer.js";
...
const parseCliArgs = (args: string[]): CliOptions => {
  let dryRun = false;
  let verbose = false;
  let sourceDir = resolve(repoRoot, "public/Men");
  let reportPath = resolve(repoRoot, ".runlogs/men-import-report.json");
...
const run = async () => {
  ...
  const report = await runMenImport({
    sourceDir: options.sourceDir,
    dryRun: options.dryRun,
    verbose: options.verbose,
  });

  await writeMenImportReport(options.reportPath, report);

  // eslint-disable-next-line no-console
  console.log(
    [
      `Men's import ${options.dryRun ? "(dry-run)" : ""} completed.`,
...
run().catch((error) => {
  console.error("Failed to import men's products:", error);
  process.exit(1);
});
