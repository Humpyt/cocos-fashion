...
import { runShoeImport, writeShoeImportReport } from "../src/utils/shoe-importer.js";
...
const parseCliArgs = (args: string[]): CliOptions => {
  let dryRun = false;
  let verbose = false;
  let sourceDir = resolve(repoRoot, "public/Ladies Shoes");
  let reportPath = resolve(repoRoot, ".runlogs/ladies-shoes-import-report.json");
...
const run = async () => {
  ...
  const report = await runShoeImport({
    sourceDir: options.sourceDir,
    dryRun: options.dryRun,
    verbose: options.verbose,
  });

  await writeShoeImportReport(options.reportPath, report);

  // eslint-disable-next-line no-console
  console.log(
    [
      `Ladies shoes import ${options.dryRun ? "(dry-run)" : ""} completed.`,
...
run().catch((error) => {
  console.error("Failed to import ladies shoes:", error);
  process.exit(1);
});
